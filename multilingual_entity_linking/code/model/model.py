...
class Model(BaseModel):
    ...
    def __init__(self, args, next_element, language):
        super().__init__(args)
        ...
        """
        with open(config.base_folder +"data/tfrecords/" + self.language + "/" + self.args.experiment_name +
    ...
    def init_embeddings(self):
        print("\n!!!! init embeddings !!!!\n")
        # read the numpy file
        embeddings_nparray = np.load(config.base_folder +"data/tfrecords/" + self.language + "/" + self.args.experiment_name +
    ...
    def add_embeddings_op(self):
        ...
        with tf.variable_scope("entities"):
            ...
            self.nentities = len(load_wikiid2nnid(self.language, extension_name=self.args.entity_extension))
    ...
    def add_cand_ent_scores_op(self):
        ...
        self.log_cand_entities_scores = tf.log(tf.maximum(self.args.zero, self.cand_entities_scores))
        stack_values = []
