...
def load_train_args(output_folder, running_mode):
    ...
    if not hasattr(train_args, 'nn_components'):
        train_args.nn_components = "pem_lstm_attention_global"
...
def load_ent_vecs(args):
    entity_embeddings_nparray = np.load(config.base_folder + "generated/"+args.language+"/ent_vecs.npy")
    # entity_embeddings_nparray[0] = 0
    #if hasattr(args, 'entity_extension') and args.entity_extension is not None:
    #    for name in args.entity_extension.split("_"):
    #        entity_extension = np.load(config.base_folder + "generated/"+name+"/ent_vecs.npy")
