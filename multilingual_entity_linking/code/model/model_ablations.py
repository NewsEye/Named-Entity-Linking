...
class Model(BaseModel):
    ...
    def __init__(self, args, next_element):
        ...
        """
        with open(config.base_folder +"data/tfrecords/" + self.args.language + "/" + self.args.experiment_name+
    ...
    def init_embeddings(self):
        print("\n!!!! init embeddings !!!!\n")
        # read the numpy file
        embeddings_nparray = np.load(config.base_folder +"data/tfrecords/" + self.args.language + "/" + self.args.experiment_name+
    ...
    def add_embeddings_op(self):
        ...
        with tf.variable_scope("entities"):
            ...
            self.nentities = len(load_wikiid2nnid(self.args.language,extension_name=self.args.entity_extension))
    ...
    def custom_pem(self, log=True, buckets_boundaries=None, bucketing_name="pem_embeddings"):
        if buckets_boundaries:
            ...
            return tf.log(tf.maximum(self.args.zero, self.cand_entities_scores))
