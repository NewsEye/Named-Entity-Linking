...
def create_training_pipelines(args):
    folder = "../data/tfrecords/" + args.language + "/" + args.experiment_name + \
             ("/allspans/" if args.all_spans_training else "/gmonly/")
    training_dataset = reader.train_input_pipeline([folder + filen + ".c" for filen in args.train_datasets], args)
...
def create_el_ed_pipelines(gmonly_flag, filenames, args):
    ...
    folder = config.base_folder+"data/tfrecords/" + args.language + "/" + args.experiment_name + ("/gmonly/" if gmonly_flag else "/allspans/")
    test_datasets = []
    for filen in filenames:
        test_datasets.append(reader.test_input_pipeline([folder+filen+".c"], args))
...
def optimal_thr_calc_aux(tp_fp_scores_labels, fn_scores):
    #print(tp_fp_scores_labels, fn_scores)
...
def train(language):
    ...
    model = Model(args, next_element, language)
    model.build()
    ...
    with model.sess as sess:
        ...
        while True:
            ...
            comparison_ed_score = ed_scores[1]
...
def _parse_args():
    ...
    parser.add_argument("--training_name", default=None,
                        help="under folder data/tfrecords/")
    parser.add_argument("--shuffle_capacity", type=int, default=200)
    ...
    parser.add_argument("--model_heads_from_bilstm", type=bool, default=True,
    ...
    parser.add_argument("--max_mention_width", type=int, default=25)
    parser.add_argument("--use_features", type=bool, default=True, help="like mention width")
    ...
    parser.add_argument("--span_emb_ffnn", default="2_100", help="int_int  the first int"
    ...
    parser.add_argument("--nn_components", default="pem_lstm_attention_global", help="each option is one scalar, then these are fed to"
    ...
    parser.add_argument("--attention_use_AB", type=bool, default=True)
    parser.add_argument("--attention_on_lstm", type=bool, default=True, help="instead of using attention on"
    ...
    parser.add_argument("--attention_ent_vecs_no_regularization", type=bool, default=True)
    ...
    parser.add_argument("--global_thr", type=float, default=-0.05)   # 0.0, 0.05, -0.05, 0.2
    ...
    if args.continue_training:
        ...
        train_args.shuffle_capacity = 200
        train_args.ed_datasets = args.ed_datasets.split('_z_') if args.ed_datasets != "" else None
        train_args.el_datasets = args.el_datasets.split('_z_') if args.el_datasets != "" else None
        train_args.train_datasets = args.train_datasets.split('_z_') if args.train_datasets != "" else None

        train_args.ed_val_datasets = [int(x) for x in args.ed_val_datasets.split('_')]
        train_args.el_val_datasets = [int(x) for x in args.el_val_datasets.split('_')]

        train_args.span_emb_ffnn = [int(x) for x in args.span_emb_ffnn.split('_')]
        train_args.final_score_ffnn = [int(x) for x in args.final_score_ffnn.split('_')]
        train_args.global_score_ffnn = [int(x) for x in args.global_score_ffnn.split('_')]
