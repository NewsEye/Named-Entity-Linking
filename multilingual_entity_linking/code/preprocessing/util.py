...
def load_entities_universe(language):
    entities_universe = set()
    # TODO this path is hardcoded. these mapping files should be transfered in ./data folder
    with open(config.base_folder+"generated/"+language+"/nnid2wikiid.txt") as fin:
        for line in fin:
            ent_id = line.strip().split('\t')[1]
            entities_universe.add(ent_id)
    return entities_universe


def load_wikiid2nnid(language, extension_name=None):
    ...
    with open(config.base_folder+"generated/"+language+"/nnid2wikiid.txt") as fin:
        for line in fin:
            nnid, ent_id = line.strip().split('\t')
            wikiid2nnid[ent_id] = int(nnid)# - 1  # torch starts from 1 instead of zero
        wikiid2nnid["<u>"] = 0
    if extension_name:
        for name in extension_name.split("_"):
            load_entity_extension(wikiid2nnid, name)
    return wikiid2nnid


def load_entity_extension(wikiid2nnid, extension_name):
    filepath = config.base_folder+"generated/"+extension_name+"/nnid2wikiid.txt"
...
def load_redirections(language, lowercase=None):
    if lowercase is None:
        lowercase = config.lowercase_maps
    wall_start = time.time()
    redirections = dict()
    with open(config.base_folder + "wiki_data/"+language+"/wiki_redirects.txt") as fin:
...
def load_disambiguations(language):
    wall_start = time.time()
    disambiguations_ids = set()
    disambiguations_errors = 0
    with open(config.base_folder + "wiki_data/"+language+"/wiki_disambiguation_pages.txt") as fin:
        for line in fin:
            line = line.rstrip()
            try:
...
def load_persons(language):
    wiki_name_id_map, _ = load_wiki_name_id_map(language)
    persons_wikiids = set()
    not_found_cnt = 0
...
def load_wiki_name_id_map(language, lowercase=False, filepath=None):
    wall_start = time.time()
    ...
    if filepath is None:
        #filepath = config.base_folder + "wiki_data/"+language+"/wiki_name_id_map.txt"
        filepath = config.base_folder + "wiki_data/"+language+"/merge-wiki_name_id_map.txt"
    disambiguations_ids = load_disambiguations(language)
    with open(filepath) as fin:
        for line in fin:
            line = line.rstrip()
            try:
...
## Ours:
def maj_min_match(mention, p_e_m, p_e_m_lower):
    if mention in p_e_m:
        return p_e_m[mention]
    elif mention.lower() in p_e_m_lower:
        return p_e_m_lower[mention.lower()]

def maj_min_concatenate_match(mention, p_e_m, p_e_m_lower):
...
def wo_punct_match(mention, p_e_m, p_e_m_lower):
    mention = ''.join(ch for ch in mention if ch not in set(string.punctuation))
    if mention in p_e_m:
        return p_e_m[mention]
    elif mention.lower() in p_e_m_lower:
        return p_e_m_lower[mention.lower()]

def wo_punct_concatenate_match(mention, p_e_m, p_e_m_lower):
    mention = ''.join(ch for ch in mention if ch not in set(string.punctuation))
    mention = mention.replace(" ", "")
    if mention in p_e_m:
        return p_e_m[mention]
...
def entity_match(mention, p_e_m, p_e_m_lower, nlp):
    span = nlp(mention)
    ents = span.ents
    if len(ents) > 0:
        ent = ents[0]
        if ent.text in p_e_m:
            return p_e_m[ent.text]
        elif ent.text.lower() in p_e_m_lower:
            return p_e_m_lower[ent.text.lower()]
    return False

def find_match(mention_original, p_e_m, p_e_m_lower, nlp, remove_stop, punctuation):
    if not punctuation:
        mention_original = unidecode(mention_original)
    mention = mention_original
    if nlp != None and remove_stop:
        mention = " ".join([ w.text for w in nlp(mention_original) if not w.is_stop])
    elif nlp != None:
        mention = " ".join([ w.text for w in nlp(mention_original)])
    probs = maj_min_match(mention, p_e_m, p_e_m_lower)
    if probs:
        return map(list, zip(*probs))
    probs = maj_min_concatenate_match(mention, p_e_m, p_e_m_lower)
    if probs:
        return map(list, zip(*probs))
    probs = wo_punct_match(mention, p_e_m, p_e_m_lower)
    if probs:
        return map(list, zip(*probs))
    probs = wo_punct_concatenate_match(mention, p_e_m, p_e_m_lower)
    if probs:
        return map(list, zip(*probs))
    #if nlp != None:
    #    probs = entity_match(mention_original, p_e_m, p_e_m_lower, nlp)
    #    if probs:
    #        return map(list, zip(*probs))
    return None, None

def disambiguate_mention(mention, nlp, p_e_m, p_e_m_lower):
    stopwords = ["_", "district","environ","un","une","uns","village de", "monsieur","madame","village","m.", "»","«", "“" "mm.","mm","d'","ville de","de","le", "les","la","rue","docteur","ville","des","lac","the","this","his","of","state","no.","mr.","dr.","mme.","m","mme","capt.","to","captain","port","gen.","and","city"]

    ents, scores = find_match(mention, p_e_m, p_e_m_lower, nlp, remove_stop=False, punctuation=True)
    if ents != None and scores != None: 
        return ents, scores
    ents, scores = find_match(mention, p_e_m, p_e_m_lower, nlp, remove_stop=False, punctuation=False)
    if ents != None and scores != None:
        return ents, scores

    mention2 = " ".join([ w for w in mention.lower().split() if w not in stopwords ])
    mention2 = "".join([ c for c in mention2 if c not in string.punctuation])
    mention2 = re.sub(' +', ' ', mention2)
    ents, scores = find_match(mention2, p_e_m, p_e_m_lower, nlp, remove_stop=False, punctuation=True)
    if ents != None and scores != None:
        return ents, scores
    ents, scores = find_match(mention2, p_e_m, p_e_m_lower, nlp, remove_stop=False, punctuation=False)
    if ents != None and scores != None:
        return ents, scores
    return None, None
...
class FetchCandidateEntities(object):
    ...
    def __init__(self, args):
        ...
        elif args.language in ["fr","de","es","pt"]:
            self.nlp = spacy.load(args.language+"_core_news_md")
        else:
            self.nlp = None
        self.p_e_m, self.p_e_m_low, self.mention_total_freq = custom_p_e_m(
            language=args.language,
            cand_ent_num=args.cand_ent_num,
            lowercase_p_e_m=True)
        # self.p_e_m, self.p_e_m_low, self.mention_total_freq = custom_p_e_m(
        #     language=args.language,
        #     cand_ent_num=args.cand_ent_num,
    ...
    def process_corrections(self, span):
        """span can be either a string or a list of words"""
        if isinstance(span, list):
            span = ' '.join(span)
        title = span.title()
        ...
        if len("".join([ c for c in mention if c not in string.punctuation])) < 3:
            return None, None
        if len(" ".join([ w for w in mention.lower().split() if w not in stopwords and w not in string.punctuation])) < 3:
            return None, None      
        ...
        if title_freq == 0 and span_freq == 0:
            if self.lowercase_spans and span.lower() in self.p_e_m:
                ...
                #return None,None
                return disambiguate_mention(span, self.nlp, self.p_e_m, self.p_e_m_low)
        else:
            if span_freq > title_freq:
                return map(list, zip(*self.p_e_m[span]))
            else:
...
class FetchFilteredCoreferencedCandEntities(object):
    def __init__(self, args):
        ...
        if args.persons_coreference:
            self.persons_wikiids = load_persons(self.args.language)
...
class EntityNameIdMap(object):
    ...
    def init_compatible_ent_id(self, language):
        self.wiki_name_id_map, self.wiki_id_name_map = load_wiki_name_id_map(language, lowercase=False)

    def init_gerbil_compatible_ent_id(self, language):
        self.wiki_name_id_map, self.wiki_id_name_map = load_wiki_name_id_map(language, lowercase=False)
        self.redirections = load_redirections(language, lowercase=False)

    def init_hyperlink2id(self, language):
        self.wiki_name_id_map, self.wiki_id_name_map = load_wiki_name_id_map(language, lowercase=False)
        self.wiki_name_id_map_l, _ = load_wiki_name_id_map(language, lowercase=True)
        self.redirections = load_redirections(language, lowercase=False)
        self.disambiguations = load_disambiguations(language)
...
def custom_p_e_m(language, cand_ent_num=15, allowed_entities_set=None,
                 lowercase_p_e_m=True):
    """Args:
    ...
    with open(config.base_folder + 'generated/' + language + '/merge-wikipedia_p_e_m.txt') as fin:
    #with open(config.base_folder + 'generated/' + language + '/wikipedia_p_e_m.txt') as fin:
            ...
            try:
                ...
                for e in entities:
                    ...
                    if not entityNameIdMap.is_valid_entity_id(ent_id):
                        ...
                        res.append((ent_id, float(score)/freq_entities))
                if res:
                    if mention in p_e_m:
