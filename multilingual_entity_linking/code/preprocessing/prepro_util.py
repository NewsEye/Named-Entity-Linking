...
#     url = 'https://dl.fbaipublicfiles.com/fasttext/vectors-crawl/cc.'+langauge+'.300.bin.gz'
#     urllib.request.urlretrieve(url, config.base_folder+"data/basic_data/wordEmbeddings/fasttext/cc."+language+".300.bin.gz")
...
class VocabularyCounter(object):
    ...
    that it processes."""
    def __init__(self, language, lowercase_emb=False):
        # import gensim
        #fasttext.util.download_model(language, if_exists='ignore')  # English
        if not path.exists(config.base_folder+"data/basic_data/wordEmbeddings/muse/wiki.multi."+language+".vec"):
            print("Embeddings missing ...")
        #self.model = fasttext.load_model(config.base_folder+"data/basic_data/wordEmbeddings/fasttext/cc."+language+".300.bin")
        # self.model = gensim.models.KeyedVectors.load_word2vec_format(config.base_folder+"data/basic_data/wordEmbeddings/fasttext/cc."+language+".300.vec", binary=False)
        self.model = gensim.models.KeyedVectors.load_word2vec_format(config.base_folder+"data/basic_data/wordEmbeddings/muse/wiki.multi."+language+".vec", binary=False)
        # self.model = gensim.models.KeyedVectors.load_word2vec_format(
        # config.base_folder+"data/basic_data/wordEmbeddings/fasttext/GoogleNews-vectors-negative300.bin", binary=True)
    ...
    def serialize(self, folder=None, name="vocab_freq.pickle"):
        if folder is None:
            folder = config.base_folder+"data/vocabulary/"+language+"/"
        if not os.path.exists(folder):
            os.makedirs(folder)
        with open(folder+name, 'wb') as handle:
            pickle.dump((self.word_freq, self.char_freq), handle)

    def count_datasets_vocabulary(self, language):
        ...
        self.serialize(folder=config.base_folder+"data/vocabulary/"+language+"/",
                       name="vocab_freq.pickle")


def build_word_char_maps(language):
    output_folder = config.base_folder+"data/tfrecords/"+language+"/"+args.experiment_name+"/"
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    with open(config.base_folder+"data/vocabulary/"+language+"/vocab_freq.pickle", 'rb') as handle:
    ...
    if not path.exists(config.base_folder+"data/basic_data/wordEmbeddings/muse/wiki.multi."+language+".vec"):
        print("Embeddings missing ...")
    #model = fasttext.load_model(config.base_folder+"data/basic_data/wordEmbeddings/fasttext/cc."+language+".300.bin")
    # model = gensim.models.KeyedVectors.load_word2vec_format(config.base_folder+"data/basic_data/wordEmbeddings/fasttext/cc."+language+".300.vec", binary=False)
    model = gensim.models.KeyedVectors.load_word2vec_format(config.base_folder+"data/basic_data/wordEmbeddings/muse/wiki.multi."+language+".vec", binary=False)
...
def build_word_char_maps_restore(language):
    output_folder = config.base_folder+"data/tfrecords/"+language+"/"+args.experiment_name+"/"
...
class SamplesGenerator(object):
    ...
    def _process_allspans(self, filepath):
        ...
        if args.calculate_stats:
            ...
            print("recall %     : ", (1 - (gm_misses+gt_misses)/(gm_this_file+1.0))*100, " %")
...
class EncoderGenerator(object):
    ...
    def __init__(self, language):
        ...
        self._word2id, self._char2id = build_word_char_maps(language)
        #self._word2id, self._char2id = build_word_char_maps_restore()  # alternative
        self._wikiid2nnid = util.load_wikiid2nnid(language, args.entity_extension)
...
class TFRecordsGenerator(object):
    def __init__(self, language):
        self._generator = EncoderGenerator(language)
    ...
    def process(self, filepath, language):
...
def create_tfrecords(language):
    new_dataset_folder = config.base_folder+"data/new_datasets/"+language+"/"
    datasets = [os.path.basename(os.path.normpath(d)) for d in util.get_immediate_files(new_dataset_folder) if language in d]
    print("datasets: ", datasets)

    tfrecords_generator = TFRecordsGenerator(language)
    tfrecords_generator.set_gmonly_mode()
    for file in datasets:
        tfrecords_generator.process(filepath=new_dataset_folder+file, language=language)
...
def create_entity_universe(language, gmonly_files=None, allspans_files=None, printSamples=None):
    new_dataset_folder = config.base_folder+"data/new_datasets/"+language+"/"
    ...
    if allspans_files is None:
        ...
        for dataset in util.get_immediate_files(new_dataset_folder):
            if language in dataset:
                allspans_file.append(os.path.basename(os.path.normpath(dataset)))
    ...
    def create_entity_universe_aux(generator, datasets):
        ...
        print("recall %     : ", (1 - (generator.all_gm_misses+generator.all_gt_misses)/(generator.all_gm+1.0))*100, " %")
    ...
    with open(config.base_folder+"data/entities/"+language+"/entities_universe.txt", "w") as fout:
...
def create_necessary_folders(language):
    if not os.path.exists(config.base_folder+"data/tfrecords/"+language+"/"):
        os.makedirs(config.base_folder+"data/tfrecords/"+language+"/")
...
def _parse_args():
    ...
    parser.add_argument("--max_mention_width", type=int, default=25, help="in allspans mode consider all spans with"
...
if __name__ == "__main__":
    args = _parse_args()
    print(args)
    create_necessary_folders(args.language)
    log_args(config.base_folder+"data/tfrecords/"+args.language+"/"+args.experiment_name+"/")

    vocabularyCounter = VocabularyCounter(args.language)
    vocabularyCounter.count_datasets_vocabulary(args.language)
    if args.create_entity_universe:
        create_entity_universe(gmonly_files=[], allspans_files=None)
    else:
        create_tfrecords(args.language)
